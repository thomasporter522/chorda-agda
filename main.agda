...
morph : (metas metas' : nat) -> Set 
morph metas metas' = Vec (pat metas') metas
...
mutual
    ...
    data compose-eq {metas metas' metas': nat} : (m1 : morph metas metas') -> (m2 : morph metas' metas'') -> (m : morph metas metas'') -> Set where 
        X-compose-eq : ∀{x p} ->
            index-eq ps x p ->
            compose-eq ps (X x) p
